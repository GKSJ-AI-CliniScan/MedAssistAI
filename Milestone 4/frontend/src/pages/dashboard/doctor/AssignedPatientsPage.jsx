...
import { Users, Search, Loader2, AlertCircle, Mail, Activity, FileText, Phone } from 'lucide-react';
...
export default function AssignedPatientsPage() {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-clinical-mutedLight dark:text-clinical-mutedDark mt-0.5">
            Active patient roster & clinical triage status
          </p>
        </div>
        <ModTag variant="brand">Physician Roster Desk</ModTag>
      </div>
      ...
      <Card title="Patient Registry" subtitle="Physician Assigned Roster">
        <div className="space-y-4">
          <p className="text-sm text-clinical-textLight dark:text-clinical-textDark leading-relaxed">
            Manage your queue of assigned patients, review health logs, and issue updated clinical plans.
          </p>
          ...
          {loading ? (
            ...
            <EmptyState 
              icon={Users}
              title={searchTerm ? 'No patients found' : 'No assigned patients yet'}
              description={searchTerm ? 'Try different search terms.' : 'Patients registered in the system will appear in your clinical care roster.'}
              className="py-10"
            />
          ) : (
                      ...
                      <div className="flex-1 space-y-2">
                        ...
                        <div className="flex items-center gap-2 pt-1">
                          <Button 
                            variant="primary" 
                            size="small" 
                            onClick={() => navigate('/doctor/history', { state: { patientId: patient.id } })} 
                            className="gap-1.5 text-xs py-1 px-3 bg-gradient-to-r from-[#2563EB] to-[#06B6D4]"
                          >
