...
import { FileText, Loader2, AlertCircle, Download, Brain, Calendar, Search } from 'lucide-react';
...
export default function DoctorReportsPage() {
  ...
  const fetchPatients = async () => {
    try {
      ...
      if (initialPatientId) {
        const found = list.find(p => p.id === Number(initialPatientId));
        if (found) {
  ...
  const fetchPatientReports = async (patientId) => {
    try {
      ...
      setError('Unable to load reports for selected patient.');
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-xs text-clinical-mutedLight dark:text-clinical-mutedDark mt-0.5">
          Laboratory diagnostics & AI analysis reports for assigned patients
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        <div className="lg:col-span-4 space-y-6">
          <Card title="Patient Selection" subtitle="Choose patient to review reports">
        ...
        <div className="lg:col-span-8 space-y-6">
          <Card 
            title="Clinical Diagnostics & Reports" 
            subtitle={selectedPatient ? `Reports for ${selectedPatient.user?.fullname || selectedPatient.fullname}` : 'Select a patient to view reports'}
          >
            <div className="space-y-4">
              <p className="text-xs text-clinical-mutedLight dark:text-clinical-mutedDark leading-relaxed">
                Review lab diagnostics, AI symptom assessments, risk scores, and clinical recommendations.
              </p>
              ...
              {!selectedPatient ? (
                <EmptyState 
                  ...
                  description="Select a patient from the list to view their medical reports."
                  className="py-12"
                />
              ) : loadingReports ? (
                ...
                <EmptyState 
                  ...
                  description="This patient has no diagnostic reports recorded yet."
                  className="py-12"
                />
              ) : (
