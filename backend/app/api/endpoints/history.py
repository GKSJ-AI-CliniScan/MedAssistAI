...
async def check_symptoms(
    ...
    # 1. Run baseline prediction service using symptoms
    predicted_diseases, risk_level, risk_score, recommendations = (
        await predict_diseases(request.symptoms)
    )
...
async def get_consultation_history(
    ...
    cursor = (
        db.consultations
        .find({"patient_id": current_user["id"]})
        .sort("created_at", -1)
    )
